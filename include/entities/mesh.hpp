...
struct Mesh {
    enum Primitive { eCube, eSphere, eQuad2D, eHeart };
    
    // 3D vertex format (position, normal, color, uv)
    struct Vertex {
        glm::vec3 position;
        glm::vec3 normal;
        glm::vec4 color;
    ...
    // 2D vertex format (position, color, uv) - no normals needed
    struct Vertex2D {
        glm::vec2 position;
        glm::vec4 color;
        glm::vec2 uv;
    ...
    // load 2D primitives (quad or heart)
    void init(Primitive primitive) {
        if (primitive == eQuad2D) {
            std::vector<Vertex2D> vertices = {
                {{0.0f, 0.0f}, {1, 1, 1, 1}, {0, 0}}, // bottom-left
                {{1.0f, 0.0f}, {1, 1, 1, 1}, {1, 0}}, // bottom-right
                {{1.0f, 1.0f}, {1, 1, 1, 1}, {1, 1}}, // top-right
                {{0.0f, 1.0f}, {1, 1, 1, 1}, {0, 1}}, // top-left
            };
        ...
        else if (primitive == eHeart) {
            
            // Heart pattern: 11 cols x 7 rows
            const char* pattern[] = {
                "  ##   ##  ",
                " #### #### ",
                "###########",
            ...
            float pixel_w = 1.0f / cols;
            float pixel_h = 1.0f / rows;
            
            // Iterate through pattern and create quads for each '#'
            for (int y = 0; y < rows; y++) {
                for (int x = 0; x < cols; x++) {
                    if (pattern[y][x] != '#') continue;
                    
                    // Define quad for this pixel
                    float left = x * pixel_w;
                    float right = (x + 1) * pixel_w;
                    float bottom = y * pixel_h;
                    float top = (y + 1) * pixel_h;
                    
                    uint32_t base_index = vertices.size();
                    
                    // add 4 vertices for this quad with proportional UV mapping
                    // each quad is mapped to its position in the heart grid, so the texture stretches across the entire heart shape
                    vertices.push_back({{left, bottom}, {1, 1, 1, 1}, {left, bottom}});   // bottom-left
                    vertices.push_back({{right, bottom}, {1, 1, 1, 1}, {right, bottom}});  // bottom-right
                    vertices.push_back({{right, top}, {1, 1, 1, 1}, {right, top}});     // top-right
                    vertices.push_back({{left, top}, {1, 1, 1, 1}, {left, top}});      // top-left
    ...
    // describe 3D vertex memory layout (position3, normal3, color4, uv2)
    // overload for 3D Vertex format
    ...
    // describe 2D vertex memory layout (position2, color4, uv2)
    // This overload is called for 2D Vertex2D format
    void describe_layout(std::vector<Vertex2D>& vertices, std::vector<uint32_t>& indices) {
        _index_count = indices.size();

        // describe vertex buffer
        GLsizeiptr vertex_byte_count = vertices.size() * sizeof(Vertex2D);
        glCreateBuffers(1, &_vertex_buffer_object);
        glNamedBufferStorage(_vertex_buffer_object, vertex_byte_count, vertices.data(), BufferStorageMask::GL_NONE_BIT);
        ...
        // create vertex array buffer
        glCreateVertexArrays(1, &_vertex_array_object);
        glVertexArrayVertexBuffer(_vertex_array_object, 0, _vertex_buffer_object, 0, sizeof(Vertex2D));
        glVertexArrayElementBuffer(_vertex_array_object, _element_buffer_object);
        
        // struct Vertex2D {
        //     glm::vec2 position; <--- 2 floats at offset 0
        //     glm::vec4 color;
        //     glm::vec2 uv;
        // };
        glVertexArrayAttribFormat(_vertex_array_object, 0, 2, GL_FLOAT, GL_FALSE, 0);
        glVertexArrayAttribBinding(_vertex_array_object, 0, 0);
        glEnableVertexArrayAttrib(_vertex_array_object, 0);
        
        // struct Vertex2D {
        //     glm::vec2 position;
        //     glm::vec4 color; <--- 4 floats at offset 2*float
        //     glm::vec2 uv;
        // };
        glVertexArrayAttribFormat(_vertex_array_object, 1, 4, GL_FLOAT, GL_FALSE, 2 * sizeof(GL_FLOAT));
        glVertexArrayAttribBinding(_vertex_array_object, 1, 0);
        glEnableVertexArrayAttrib(_vertex_array_object, 1);
        ...
        //     glm::vec2 position;
        //     glm::vec4 color;
        //     glm::vec2 uv; <--- 2 floats at offset 6*float
        // };
        glVertexArrayAttribFormat(_vertex_array_object, 2, 2, GL_FLOAT, GL_FALSE, 6 * sizeof(GL_FLOAT));
        glVertexArrayAttribBinding(_vertex_array_object, 2, 0);
        glEnableVertexArrayAttrib(_vertex_array_object, 2);
    }
